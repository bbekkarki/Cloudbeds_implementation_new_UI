...
const AssignRoom = () => {
  const [isRoomAssigned, setIsRoomAssigned] = useState(false);
  const [unassignedRoom, setUnassignedRoom] = useState(null);
  const [roomsOptions, setRoomsOptions] = useState([]);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    ...
    const checkRoomAssignment = async () => {
      ...
      try {
        ...
        const reservationData = response.data.data;
        const unassignedRooms = reservationData.unassigned;

        if (unassignedRooms.length === 0) {
          setIsRoomAssigned(true);
          navigate("/sign-user");
        } else {
          const nextUnassigned = unassignedRooms[0];
          setUnassignedRoom(nextUnassigned);

          await fetchAvailableRooms(
            nextUnassigned.roomTypeID,
            nextUnassigned.roomTypeNameShort,
            nextUnassigned.startDate,
            nextUnassigned.endDate
          );
        }
      } catch (error) {
        console.error("Error fetching reservation details:", error);
  ...
  const handleRoomClick = async (room) => {
    ...
    try {
      ...
      if (response.data.success) {
        toast.success("Room assigned successfully!");
        setReload((prev) => !prev); // Reload to assign next room
      } else {
        toast.error(response.data.message || "Room assignment failed!");
        setError(response.data.message || "Room assignment failed.");
      }
    } catch (error) {
      console.error("Error assigning room:", error);
      toast.error("Error assigning room!");
